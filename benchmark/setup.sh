...
conda run -n "ase-mace" pip cache purge
conda run -n "ase-mace" pip install torch-ema torchmetrics
conda run -n "ase-mace" pip install --force-reinstall --no-cache-dir torch torchvision torchaudio \
...
conda run -n "ase-aceff" pip uninstall torch torchvision torchaudio -y 
conda run -n "ase-aceff" pip install --pre torch --index-url https://download.pytorch.org/whl/nightly/cu128
conda run -n "ase-aceff" pip uninstall torch torchvision torchaudio -y
conda run -n "ase-aceff" pip install --pre torch --index-url https://download.pytorch.org/whl/nightly/cu128
...
###############################################################################
# 12. openmm-ml  (ANI-1ccx, ANI-2x, MACE-OFF/MPA/OMAT/OMOL, AceFF, AIMNet2,
#                 Orb-v3, FeNNix-Bio1)
###############################################################################

make_env "openmm-ml"
# OpenMM itself must come from conda-forge (no pip wheel)
conda install -n "openmm-ml" -c conda-forge openmm -y -q
install_torch "openmm-ml"
conda run -n "openmm-ml" pip install -q \
    openmmml \
    torchani \
    mace-torch \
    cuequivariance_torch \
    torchmd-net \
    "aimnet[ase]" \
    orb-models pynanoflann \
    "fennol[cuda]"
echo "[INFO] openmm-ml: done"
...
run_test "openmm-ml"   "import openmm, openmmml, torchani, mace, torchmdnet, aimnet, orb_models, fennol; print('ok  openmm=' + openmm.__version__)"
