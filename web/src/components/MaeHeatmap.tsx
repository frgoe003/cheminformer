...
interface TooltipData { model: string; col: string; value: number }
...
export function MaeHeatmap({ matrix }: { matrix: MaeMatrix }) {
  const { models, cols, counts, data, min, max } = matrix;
                ...
                {cols.map((col) => {
                  ...
                  return (
                    <td
                      ...
                      onMouseEnter={(e) =>
                        val !== null &&
                        setHover({ data: { model, col, value: val }, pos: { x: e.clientX, y: e.clientY } })
                      }
                      onMouseMove={(e) =>
